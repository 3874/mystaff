...
$(async function () {
    ...
    $("#testBtn").on("click", async function () {
        ...
        try {
            // Show loading state
            const pageFrame = $("#pageFrame")[0];
            const pageDoc = pageFrame.contentDocument || pageFrame.contentWindow.document;
            pageDoc.open();
            pageDoc.write("<h1>Loading...</h1>");
            pageDoc.close();

            const response = await handleMsg(processedInput, responder, sessionId);

            // Try to extract HTML content from response
            let htmlContent = "";
            ...
            const pageDoc = pageFrame.contentDocument || pageFrame.contentWindow.document;
            pageDoc.open();
            pageDoc.write(`<h1>Error</h1><p>${error.message}</p>`);
            pageDoc.close();
