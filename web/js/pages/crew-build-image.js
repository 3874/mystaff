...
$(async function () {
    ...
    const adapterConfigs = {
        ...
        gemini: {
            host: "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro-vision:generateContent",
            authorization: "Bearer ",
            contentType: "application/json",
        },
    };
    ...
    $("#adapter_name").on("change", function () {
        ...
        if (config) {
            ...
            if (selectedAdapter === "openai") {
                $("#adapter_model").val("dall-e-3");
            } else if (selectedAdapter === "gemini") {
                $("#adapter_model").val("gemini-pro-vision");
            }
        }
    });
    ...
    $("#testBtn").on("click", async function () {
        ...
        try {
            $("#output").val("Testing...");
            $("#imagePlaceholder").text("Generating image...");
            $("#outputImage").hide();
            ...
            if (typeof response === 'string' && response.startsWith('http')) {
                ...
                imageUrl = response.data[0].url; // OpenAI format
            } else if (response.url) {
                imageUrl = response.url;
            }

            if (imageUrl) {
                $("#outputImage").attr("src", imageUrl).show();
                $("#imagePlaceholder").hide();
            } else {
                $("#imagePlaceholder").text("No image URL found in response.");
                $("#outputImage").hide();
            }
            ...
            $("#imagePlaceholder").text("Error generating image.");
